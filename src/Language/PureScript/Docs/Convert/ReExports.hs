...
collectDeclarations reExports = do
  ...
  expCtors =
    concatMap (getCtors . snd) reExports
    where
    getCtors :: P.DeclarationRef -> [P.ProperName 'P.ConstructorName]
    getCtors (P.TypeRef _ _ ctors) = fromMaybe [] ctors
    getCtors _ = []
